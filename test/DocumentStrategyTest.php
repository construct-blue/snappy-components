...
class DocumentStrategyTest extends TestCase
{
    ...
    public function testShouldRenderResourcesInHead()
    {
        $renderer = new Renderer(new DocumentStrategy(new Document('en', 'example')));
        $result = $renderer->render(new Elements([
            '<h1>example</h1>',
            new Resource('test')
        ]), (object)[]);
        self::assertEquals('<!DOCTYPE html><html lang="en"><head><title>example</title>test</head><body><h1>example</h1></body>', $result);
    }
}
